...
PYBIND11_MODULE(_packSpheres,mod){
	...
	py::class_<SpherePack PY_SHARED_PTR_HOLDER(SpherePack)>(mod,"SpherePack","Set of spheres represented as centers and radii. This class is returned by :obj:`woo.pack.randomDensePack`, :obj:`woo.pack.randomPeriPack` and others. The object supports iteration over spheres, as in \n\n\t>>> sp=SpherePack()\n\t>>> for center,radius in sp: print center,radius\n\n\t>>> for sphere in sp: print sphere[0],sphere[1]   ## same, but without unpacking the tuple automatically\n\n\t>>> for i in range(0,len(sp)): print sp[i][0], sp[i][1]   ## same, but accessing spheres by index\n\nClumps are supported, by using the *clumpId* parameter to :obj:`add`.\n\n\n.. admonition:: Special constructors\n\n\tConstruct from list of ``[(c1,r1),(c2,r2),…]``. To convert two same-length lists of ``centers`` and ``radii``, construct with ``zip(centers,radii)``.\n")
		...
		.def("pruneOverlapping",&SpherePack::pruneOverlapping,py::arg("minRelOverlap"),"Remove all spheres with some minimal relative overlap; of the two overlapping spheres, the larger one will be removed. Returns the number of spheres removed.")
