...
PYBIND11_MODULE(_utils2,mod){
	...
	mod.def("mapColor",CompUtils::scalarOnColorScale,py::arg("x"),py::arg("min")=0,py::arg("max")=1,py::arg("cmap")=-1,py::arg("reversed")=false,"Map scalar to color (as 3-tuple). See :obj:`woo.core.Master.cmap`, :obj:`woo.core.Master.cmaps` to set colormap globally.");
	mod.def("unbalancedForce",unbalancedForce,py::arg("scene")=shared_ptr<Scene>(),py::arg("useMaxForce")=false,"Compute the ratio of mean (or maximum, if *useMaxForce*) summary force on bodies and mean force magnitude on interactions. It is an adimensional measure of staticity, which approaches zero for quasi-static states.");
	mod.def("facetsPlaneIntersectionSegments",facetsPlaneIntersectionSegments,py::arg("facets"),py::arg("pt"),py::arg("normal"),"Return list of points, where consecutive pairs are segment where *facets* were intersecting plane given by *pt* and *normal*.");
	mod.def("outerTri2Dist",outerTri2Dist,py::arg("pt"),py::arg("A"),py::arg("B"),py::arg("C"),"Return signed distance of point *pt* in triangle A,B,C. The result is distance of point *pt* to the closest point on triangle A,B,C. The distance is negative is *pt* is inside, 0 if exactly on the triangle and positive outside. Signedness supposes that A,B,C are given anti-clockwise; otherwise, the sign will be reversed");
	mod.def("flipCell",flipCell,py::arg("scene"),py::arg("flip")=Matrix3r::Zero().eval(),"Flip periodic cell (by default to the state the closest to the canonical (orthogonal) state) without affecting contacts.");
	...
	mod.def("contactCoordQuantiles",DemFuncs::contactCoordQuantiles,py::arg("dem"),py::arg("quantiles"),py::arg("node")=shared_ptr<Node>(),py::arg("box")=AlignedBox3r(),"Return list of (local) contact z-coordinates for given quantile values; if *node* is omited, global coordinates are used; *box* is specified in node-local coordinates; if *box* is empty (default-initialized), all contacts are included.");
	mod.def("coordNumber",DemFuncs::coordNumber,py::arg("dem"),py::arg("node")=shared_ptr<Node>(),py::arg("box")=AlignedBox3r(Vector3r(-Inf,-Inf,-Inf),Vector3r(Inf,Inf,Inf)),py::arg("mask")=0,py::arg("skipFree")=true,"Return coordination number of the sampled area given by optional local coordinates (*node*) and *box* (which is in local coordinates if *node* is given and global if not); if *mask* is non-zero, particles without matching :obj:`mask <woo.dem.Particle.mask>` are ignored. See `yade.utils.avgNumInterations <https://yade-dem.org/doc/yade.utils.html#yade.utils.avgNumInteractions>`__ for formulas on skipping free particles (TODO: copy those over).\n\n.. warning:: Clumps are not handled properly by this function and an exception will be raised if they are encountered.");
	mod.def("porosity",DemFuncs::porosity,py::arg("dem"),py::arg("node")=shared_ptr<Node>(),py::arg("box"),"Return `porosity <http://en.wikipedia.org/wiki/Porosity>`__ for given box (in local coordinates, if *node* is not ``None``), considering (only) spherical particles of which centroid is in the box.");
	#ifdef WOO_VTK
		mod.def("vtkExportTraces",DemFuncs::vtkExportTraces,py::arg("scene"),py::arg("dem"),py::arg("filename"),py::arg("moduloOffset")=Vector2i(0,0),"Export traces of existing spheroid particles to VTK file, for visualization in Paraview.");
	#endif
}
