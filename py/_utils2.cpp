...
BOOST_PYTHON_MODULE(_utils2){
	...
	py::def("flipCell",flipCell,(py::arg("scene"),py::arg("flip")=Matrix3r::Zero().eval()),"Flip periodic cell (by default to the state the closest to the canonical (orthogonal) state) without affecting contacts.");
	...
	py::def("contactCoordQuantiles",DemFuncs::contactCoordQuantiles,(py::arg("dem"),py::arg("quantiles"),py::arg("node")=shared_ptr<Node>(),py::arg("box")=AlignedBox3r()),"Return list of (local) contact z-coordinates for given quantile values; if *node* is omited, global coordinates are used; *box* is specified in node-local coordinates; if *box* is empty (default-initialized), all contacts are included.");
	...
	#ifdef WOO_VTK
		py::def("vtkExportTraces",DemFuncs::vtkExportTraces,(py::arg("scene"),py::arg("dem"),py::arg("filename"),py::arg("moduloOffset")=Vector2i(0,0)),"Export traces of existing spheroid particles to VTK file, for visualization in Paraview.");
	#endif
}
