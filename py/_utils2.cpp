...
Matrix3r flipCell(const shared_ptr<Scene>& _scene, const Matrix3r& flip){
	// Matrix3i is not wrapped in minieigen (or not everywhere), so convert back and forth
	Matrix3i flipI=flip.cast<int>();
	if(flipI.cast<Real>()!=flip) throw std::runtime_error("flipCell: *flip* must only have zero entries (float matrix type is used as workaround only).");
	if(!_scene) throw std::runtime_error("flipCell: None Scene object passed.");
	return DemFuncs::flipCell(_scene.get(),flipI).cast<Real>();
}
...
#ifdef WOO_PYBIND11
PYBIND11_MODULE(_utils2,mod){
	mod.attr("__name__")="woo._utils2";
	#define _PY_MOD mod.
#else
BOOST_PYTHON_MODULE(_utils2){
	...
	_PY_MOD def("pWaveDt",pWaveDt,WOO_PY_ARGS(py::arg("scene")=py::object(),py::arg("noClumps")=false),"Get timestep accoring to the velocity of P-Wave propagation; computed from sphere radii, rigidities and masses.");
	_PY_MOD def("reactionInPoint",reactionInPoint,WOO_PY_ARGS(py::arg("dem"),py::arg("mask"),py::arg("pt"),py::arg("multinodal")=true),"Compute force and torque in point *pt* generated by forces on particles matching *mask* in :obj:`DemField` *dem*. Multinodal particles (such as :obj:`facets <Facet>`) usually don't nodal forces evaluated; their contacts are traversed and contact reaction computed directly if *multinodal* is ``True`` (in that case, multinodal-multinodal contacts should not be present at all). Return tuple ``(force,torque,N)``, where ``N`` is the number of particles matched by *mask*.");
	_PY_MOD def("particleStress",particleStress,"Return tuple of normal and shear stresses acting on given particle; raise exception for non-spherical particles");
	_PY_MOD def("boxPsd",boxPsd,WOO_PY_ARGS(py::arg("box")=AlignedBox3r(Vector3r(NaN,NaN,NaN),Vector3r(NaN,NaN,NaN)),py::arg("mass")=true,py::arg("num")=20,py::arg("mask")=0,py::arg("dRange")=Vector2r(0.,0.),py::arg("zip")=true),"Compute Particle size distribution in given box (min,max) or in the whole simulation, if box is not specified; list of couples (diameter,passing) is returned; with *unzip*, tuple of two sequences, diameters and passing values, are returned.");
	_PY_MOD def("psd",psd,WOO_PY_ARGS(py::arg("ddmm"),py::arg("mass")=true,py::arg("cumulative")=true,py::arg("normalize")=false,py::arg("dRange")=Vector2r(NaN,NaN),py::arg("num")=80),"Return PSD of given particles (given as array of Vector2, where each Vector2 specifies diameter and mass) as arrays of diameters and passing; the semantics is the same as for :obj:`woo.dem.BoxDeleter.psd` and :obj:`woo.dem.ParticleGenerator.psd`.");
	_PY_MOD def("createContacts",createContacts,WOO_PY_ARGS(py::arg("ids1"),py::arg("id2s"),py::arg("geomFunctors")=vector<shared_ptr<CGeomFunctor> >(),py::arg("physFunctors")=vector<shared_ptr<CPhysFunctor> >(),py::arg("force")=true),"Create contacts between given DEM particles.\n\nCurrent engines are searched for :obj:`woo.dem.ContactLoop`, unless *geomFunctors* and *physFunctors* are given. *force* will make :obj:`woo.dem.CGeomFunctor` acknowledge the contact even if particles don't touch geometrically.\n\n.. warning::\n\tThis function will very likely behave incorrectly for periodic simulations (though it could be extended it to handle it farily easily).");
	_PY_MOD def("stressStiffnessWork",stressStiffnessWork,WOO_PY_ARGS(py::arg("volume")=0,py::arg("skipMultinodal")=true,py::arg("prevStress")=(Vector6r()<<NaN,NaN,NaN,NaN,NaN,NaN).finished()),"Compute stress and stiffness tensors, and work increment of current velocity gradient (*nan* for aperiodic simulations); returns tuple (stress, stiffness, work), where stress and stiffness are in Voigt notation. *skipMultinodal* skips all contacts involving particles with multiple nodes, where stress & stiffness values can be determined only by In2 functors.");
	_PY_MOD def("muStiffnessScaling",muStiffnessScaling,WOO_PY_ARGS(py::arg("piHat")=M_PI/2,py::arg("skipFloaters")=false,py::arg("V")=-1),"Compute stiffness scaling parameter relating continuum-like stiffness with packing stiffness; see 'Particle assembly with cross-anisotropic stiffness tensor' for details. With *skipFloaters*, ignore contacts where any of the two contacting particlds has only one *real* contact (thus not contributing to the assembly stability).");
	_PY_MOD def("bestFitCompliance",bestFitCompliance,"Compute compliance based on best-fit hypothesis, using the paper [Liao1997], equations (30) and (28,31).");
	_PY_MOD def("mapColor",CompUtils::scalarOnColorScale,WOO_PY_ARGS(py::arg("x"),py::arg("min")=0,py::arg("max")=1,py::arg("cmap")=-1,py::arg("reversed")=false),"Map scalar to color (as 3-tuple). See :obj:`woo.core.Master.cmap`, :obj:`woo.core.Master.cmaps` to set colormap globally.");
	_PY_MOD def("unbalancedForce",unbalancedForce,WOO_PY_ARGS(py::arg("scene")=shared_ptr<Scene>(),py::arg("useMaxForce")=false),"Compute the ratio of mean (or maximum, if *useMaxForce*) summary force on bodies and mean force magnitude on interactions. It is an adimensional measure of staticity, which approaches zero for quasi-static states.");
	_PY_MOD def("facetsPlaneIntersectionSegments",facetsPlaneIntersectionSegments,WOO_PY_ARGS(py::arg("facets"),py::arg("pt"),py::arg("normal")),"Return list of points, where consecutive pairs are segment where *facets* were intersecting plane given by *pt* and *normal*.");
	_PY_MOD def("outerTri2Dist",outerTri2Dist,WOO_PY_ARGS(py::arg("pt"),py::arg("A"),py::arg("B"),py::arg("C")),"Return signed distance of point *pt* in triangle A,B,C. The result is distance of point *pt* to the closest point on triangle A,B,C. The distance is negative is *pt* is inside, 0 if exactly on the triangle and positive outside. Signedness supposes that A,B,C are given anti-clockwise; otherwise, the sign will be reversed");
	_PY_MOD def("flipCell",flipCell,WOO_PY_ARGS(py::arg("scene"),py::arg("flip")=Matrix3r::Zero().eval()),"Flip periodic cell (by default to the state the closest to the canonical (orthogonal) state) without affecting contacts.");
	_PY_MOD def("importSTL",DemFuncs::importSTL,WOO_PY_ARGS(py::arg("filename"),py::arg("mat"),py::arg("mask")=(int)DemField::defaultBoundaryMask,py::arg("color")=-.999,py::arg("scale")=1.,py::arg("shift")=Vector3r::Zero().eval(),py::arg("ori")=Quaternionr::Identity(),py::arg("threshold")=-1e-6,py::arg("maxBox")=NaN,py::arg("readColors")=true,py::arg("flex")=false,py::arg("thickness")=0.),"Return list of :obj:`particles <woo.dem.Particle>` (with the :obj:`Facet <woo.dem.Facet>` :obj:`shape <woo.dem.Particle.shape>`, or :obj:`~woo.fem.Membrane` if *flex* is ``True``) imported from given STL file.\nBoth ASCII and binary formats are supported; ``COLOR`` and ``MATERIAL`` values in the binary format, if given and with *readColors*, are read but currently ignored (they should translate to the :obj:`woo.dem.Shape.color` scalar − that color difference would be preserved, but not the color as such), and a warning is issued.\n*scale*, *shift* and *ori* are applied in this order before :obj:`nodal <woo.core.Node>` coordinates are computed. The *threshold* value serves for merging incident vertices: if positive, it is distance in the STL space (before scaling); if negative, it is relative to the max bounding box size of the entire mesh. *maxBox*, if positive, will cause any faces to be tesselated until the smallest dimension of its bbox (in simulation space) is smaller than *maxBox*; this is to avoid many spurious (potential) contacts with large obliquely-oriented faces.\n\nIf *thickness* is non-zero, all created particles (:obj:`~woo.dem.Facet` or :obj:`~woo.fem.Membrane`) will have their :obj:`~woo.dem.Facet.halfThick` set, and node's mass and inertia will be computed automatically.");
	_PY_MOD def("contactCoordQuantiles",DemFuncs::contactCoordQuantiles,WOO_PY_ARGS(py::arg("dem"),py::arg("quantiles"),py::arg("node")=shared_ptr<Node>(),py::arg("box")=AlignedBox3r()),"Return list of (local) contact z-coordinates for given quantile values; if *node* is omited, global coordinates are used; *box* is specified in node-local coordinates; if *box* is empty (default-initialized), all contacts are included.");
	_PY_MOD def("coordNumber",DemFuncs::coordNumber,WOO_PY_ARGS(py::arg("dem"),py::arg("node")=shared_ptr<Node>(),py::arg("box")=AlignedBox3r(Vector3r(-Inf,-Inf,-Inf),Vector3r(Inf,Inf,Inf)),py::arg("mask")=0,py::arg("skipFree")=true),"Return coordination number of the sampled area given by optional local coordinates (*node*) and *box* (which is in local coordinates if *node* is given and global if not); if *mask* is non-zero, particles without matching :obj:`mask <woo.dem.Particle.mask>` are ignored. See `yade.utils.avgNumInterations <https://yade-dem.org/doc/yade.utils.html#yade.utils.avgNumInteractions>`__ for formulas on skipping free particles (TODO: copy those over).\n\n.. warning:: Clumps are not handled properly by this function and an exception will be raised if they are encountered.");
	_PY_MOD def("porosity",DemFuncs::porosity,WOO_PY_ARGS(py::arg("dem"),py::arg("node")=shared_ptr<Node>(),py::arg("box")),"Return `porosity <http://en.wikipedia.org/wiki/Porosity>`__ for given box (in local coordinates, if *node* is not ``None``), considering (only) spherical particles of which centroid is in the box.");
	#ifdef WOO_VTK
		_PY_MOD def("vtkExportTraces",DemFuncs::vtkExportTraces,WOO_PY_ARGS(py::arg("scene"),py::arg("dem"),py::arg("filename"),py::arg("moduloOffset")=Vector2i(0,0)),"Export traces of existing spheroid particles to VTK file, for visualization in Paraview.");
	#endif
}
