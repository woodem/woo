...
struct PredicateWrap: public Predicate{
	NB_TRAMPOLINE(Predicate,20);
	// using Predicate::Predicate;
	bool operator()(const Vector3r& pt, Real pad=0.) const override {
		PYBIND11_OVERRIDE_PURE_NAME(/*return type*/bool,/*parent class*/Predicate,/*python name*/"__call__",/*c++ function*/operator(),/*args*/pt,pad);
	}
	AlignedBox3r aabb() const override {
		PYBIND11_OVERRIDE_PURE(/*return type*/AlignedBox3r,/*parent class*/Predicate,/*function*/aabb);
	}
};
...
PYBIND11_MODULE(_packPredicates,mod){
	...
	// boolean operations
	py::class_<PredicateBoolean,Predicate PY_SHARED_PTR_HOLDER(PredicateBoolean)>(mod,"PredicateBoolean","Boolean operation on 2 predicates (abstract class)")
		.def_property_readonly("A",&PredicateBoolean::getA).def_property_readonly("B",&PredicateBoolean::getB);
	py::class_<PredicateUnion,PredicateBoolean PY_SHARED_PTR_HOLDER(PredicateUnion)>(mod,"PredicateUnion","Union (non-exclusive disjunction) of 2 predicates. A point has to be inside any of the two predicates to be inside. Can be constructed using the ``|`` operator on predicates: ``pred1 | pred2``.").def(py::init<shared_ptr<Predicate>,shared_ptr<Predicate>>(),py::arg("pred1"),py::arg("pred2"));
	py::class_<PredicateIntersection,PredicateBoolean PY_SHARED_PTR_HOLDER(PredicateIntersection)>(mod,"PredicateIntersection","Intersection (conjunction) of 2 predicates. A point has to be inside both predicates. Can be constructed using the ``&`` operator on predicates: ``pred1 & pred2``.").def(py::init<shared_ptr<Predicate>,shared_ptr<Predicate>>(),py::arg("pred1"),py::arg("pred2"));
	py::class_<PredicateDifference,PredicateBoolean PY_SHARED_PTR_HOLDER(PredicateDifference)>(mod,"PredicateDifference","Difference (conjunction with negative predicate) of 2 predicates. A point has to be inside the first and outside the second predicate. Can be constructed using the ``-`` operator on predicates: ``pred1 - pred2``.").def(py::init<shared_ptr<Predicate>,shared_ptr<Predicate>>(),py::arg("pred1"),py::arg("pred2"));
	py::class_<PredicateSymmetricDifference,PredicateBoolean PY_SHARED_PTR_HOLDER(PredicateSymmetricDifference)>(mod,"PredicateSymmetricDifference","SymmetricDifference (exclusive disjunction) of 2 predicates. A point has to be in exactly one predicate of the two. Can be constructed using the ``^`` operator on predicates: ``pred1 ^ pred2``.").def(py::init<shared_ptr<Predicate>,shared_ptr<Predicate>>(),py::arg("pred1"),py::arg("pred2"));
	// primitive predicates
	py::class_<inSphere PY_SHARED_PTR_HOLDER(inSphere),Predicate>(mod,"inSphere","Sphere predicate.").def(py::init<const Vector3r&,Real>(),py::arg("center"),py::arg("radius"),"Ctor taking center and radius");
	py::class_<inAlignedBox PY_SHARED_PTR_HOLDER(inAlignedBox),Predicate>(mod,"inAlignedBox","Axis-aligned box predicate").def(py::init<const Vector3r&,const Vector3r&>(),py::arg("minCorner"),py::arg("maxCorner"),"Ctor taking minumum and maximum points of the box.").def(py::init<const AlignedBox3r&>(),py::arg("box"));
	py::class_<inOrientedBox PY_SHARED_PTR_HOLDER(inOrientedBox),Predicate>(mod,"inOrientedBox","Arbitrarily oriented box specified as local coordinates (pos,ori) and aligned box in those local coordinates.").def(py::init<const Vector3r&,const Quaternionr&, const AlignedBox3r&>(),py::arg("pos"),py::arg("ori"),py::arg("box"),"Ctor taking position and orientation of the local system, and aligned box in local coordinates.");
	py::class_<inCylSector PY_SHARED_PTR_HOLDER(inCylSector),Predicate>(mod,"inCylSector","Sector of an arbitrarily oriented cylinder in 3d, limiting cylindrical coordinates :math:`\\rho`, :math:`\\theta`, :math:`z`; all coordinate limits are optional.").def(py::init<const Vector3r&, const Quaternionr&, const Vector2r&, const Vector2r&, const Vector2r&>(),py::arg("pos"),py::arg("ori"),py::arg("rrho")=Vector2r::Zero().eval(),py::arg("ttheta")=Vector2r::Zero().eval(),py::arg("zz")=Vector2r::Zero().eval(),"Ctor taking position and orientation of the local system, and aligned box in local coordinates.");
	py::class_<inAlignedHalfspace PY_SHARED_PTR_HOLDER(inAlignedHalfspace),Predicate>(mod,"inAlignedHalfspace","Half-space given by coordinate at some axis.").def(py::init<int, const Real&, bool>(),py::arg("axis"),py::arg("coord"),py::arg("lower")=true,"Ctor taking axis (0,1,2 for :math:`x`, :math:`y`, :math:`z` respectively), the coordinate along that axis, and whether the *lower* half (or the upper half, if *lower* is false) is considered.");
	py::class_<inAxisRange PY_SHARED_PTR_HOLDER(inAxisRange),Predicate>(mod,"inAxisRange","Range of coordinate along some axis, effectively defining two axis-aligned enclosing planes.").def(py::init<int, const Vector2r&>(),py::arg("axis"),py::arg("range"),"Ctor taking axis (0,1,2 for :math:`x`, :math:`y`, :math:`z` respectively), and coordinate range along that axis.");
	py::class_<inParallelepiped PY_SHARED_PTR_HOLDER(inParallelepiped),Predicate>(mod,"inParallelepiped","Parallelepiped predicate").def(py::init<const Vector3r&,const Vector3r&, const Vector3r&, const Vector3r&>(),py::arg("o"),py::arg("a"),py::arg("b"),py::arg("c"),"Ctor taking four points: ``o`` (for origin) and then ``a``, ``b``, ``c`` which define endpoints of 3 respective edges from ``o``.");
	py::class_<inCylinder PY_SHARED_PTR_HOLDER(inCylinder),Predicate>(mod,"inCylinder","Cylinder predicate").def(py::init<const Vector3r&,const Vector3r&,Real>(),py::arg("centerBottom"),py::arg("centerTop"),py::arg("radius"),"Ctor taking centers of the lateral walls and radius.").def("__str__",&inCylinder::__str__);
	py::class_<inHyperboloid PY_SHARED_PTR_HOLDER(inHyperboloid),Predicate>(mod,"inHyperboloid","Hyperboloid predicate").def(py::init<const Vector3r&,const Vector3r&,Real,Real>(),py::arg("centerBottom"),py::arg("centerTop"),py::arg("radius"),py::arg("skirt"),"Ctor taking centers of the lateral walls, radius at bases and skirt (middle radius).");
	py::class_<inEllipsoid PY_SHARED_PTR_HOLDER(inEllipsoid),Predicate>(mod,"inEllipsoid","Ellipsoid predicate").def(py::init<const Vector3r&,const Vector3r&>(),py::arg("centerPoint"),py::arg("abc"),"Ctor taking center of the ellipsoid and its 3 radii.");
	py::class_<notInNotch PY_SHARED_PTR_HOLDER(notInNotch),Predicate>(mod,"notInNotch","Outside of infinite, rectangle-shaped notch predicate").def(py::init<const Vector3r&,const Vector3r&,const Vector3r&,Real>(),py::arg("centerPoint"),py::arg("edge"),py::arg("normal"),py::arg("aperture"),"Ctor taking point in the symmetry plane, vector pointing along the edge, plane normal and aperture size.\nThe side inside the notch is edge×normal.\nNormal is made perpendicular to the edge.\nAll vectors are normalized at construction time.");
	#ifdef WOO_GTS
		py::class_<inGtsSurface PY_SHARED_PTR_HOLDER(inGtsSurface),Predicate>(mod,"inGtsSurface","GTS surface predicate").def(py::init<py::object,bool>(),py::arg("surface"),py::arg("noPad")=false,"Ctor taking a gts.Surface() instance, which must not be modified during instance lifetime.\nThe optional noPad can disable padding (if set to True), which speeds up calls several times.\nNote: padding checks inclusion of 6 points along +- cardinal directions in the pad distance from given point, which is not exact.")
