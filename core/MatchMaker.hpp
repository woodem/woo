...
class MatchMaker: public Object {
	...
	public:
		...
		#define woo_core_MatchMaker__CLASS_BASE_DOC_ATTRS_CTOR_PY \
			MatchMaker,Object,"Class matching pair of ids to return pre-defined (for a pair of ids defined in :obj:`matches`) or derived value (computed using :obj:`algo`) of a scalar parameter. It can be called (``id1``, ``id2``, ``val1=NaN``, ``val2=NaN``) in both python and c++. \n\n.. note:: There is a :obj:`converter <woo._customConverters>` from python number defined for this class, which creates a new :obj:`MatchMaker` returning the value of that number; instead of giving the object instance therefore, you can only pass the number value and it will be converted automatically.", \
