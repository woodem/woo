...
struct PeriIsoCompressor: public Engine{
	...
	void run() override;
	#define woo_dem_PeriIsoCompressor__CLASS_BASE_DOC_ATTRS \
		PeriIsoCompressor,Engine,ClassTrait().doc("Compress/decompress cloud of spheres by controlling periodic cell size until it reaches prescribed average stress, then moving to next stress value in given stress series.").section("Stress/strain control","TODO",{"WeirdTriaxControl"}), \
	...
	WOO_DECL__CLASS_BASE_DOC_ATTRS(woo_dem_PeriIsoCompressor__CLASS_BASE_DOC_ATTRS);
	WOO_DECL_LOGGER;
};
...
struct WeirdTriaxControl: public Engine{
	...
	WOO_DECL__CLASS_BASE_DOC_ATTRS(woo_dem_WeirdTriaxControl__CLASS_BASE_DOC_ATTRS);
	WOO_DECL_LOGGER;
};
