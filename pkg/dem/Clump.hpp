...
struct SphereClumpGeom: public ShapeClump {
	...
	#define woo_dem_SphereClumpGeom__CLASS_BASE_DOC_ATTRS_PY \
		...
		, /* py */ \
		.def_static("fromSpherePack",&SphereClumpGeom::fromSpherePack,WOO_PY_ARGS(py::arg("pack"),py::arg("div")=5),"Return [ :obj:`SphereClumpGeom` ] which contain all clumps and spheres from given :obj:`SpherePack`.").staticmethod("fromSpherePack") \
...
struct ClumpData: public DemData{
	...
	#define woo_dem_ClumpData__CLASS_BASE_DOC_ATTRS_PY \
		...
		,/*py*/ .def_static("forceTorqueFromMembers",&ClumpData::pyForceTorqueFromMembers,"Return the tuple (F,T), summary force and torque values collected from clump members, as acting on the clump node passed as argument.").staticmethod("forceTorqueFromMembers").def_static("applyToMembers",&ClumpData::pyApplyToMembers,"Move/rotate member nodes; this is only useful after setting clump's node position/orientation by hand, so that memebrs are updated without running Leapfrog.").staticmethod("applyToMembers")
