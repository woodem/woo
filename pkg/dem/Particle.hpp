...
struct Particle: public Object{
	...
	#define woo_dem_Particle__CLASS_BASE_DOC_ATTRS_PY \
		...
		.def_static("make",&Particle::make,(py::arg("shape"),py::arg("mat"),py::arg("fixed")=false),"Return Particle instance created from given shape and material; nodes with DemData are automatically added to the shape, mass and inertia is recomuted.").staticmethod("make") \
		; \
...
public:
	...
	#define woo_dem_DemData__CLASS_BASE_DOC_ATTRS_PY \
		...
		.add_property("useAsphericalLeapfrog",&DemData::useAsphericalLeapfrog,"Say whether the node will use aspherical :obj:`leapfrog <woo.dem.Leapfrog>` integration routine. The criterion is that the node :obj:`is aspherical <isAspherical>` **and** does not have all rotations :obj:`blocked`.") \
		.def_static("setOriMassInertia",&DemData::setOriMassInertia,"Lump mass and inertia from all attached particles and attempt to rotate the node so that its axes are principal axes of inertia, if allowed by particles shape (without chaning the geometry).").staticmethod("setOriMassInertia") \
		...
		.def_static("_getDataOnNode",&Node::pyGetData<DemData>).staticmethod("_getDataOnNode").def("_setDataOnNode",&Node::pySetData<DemData>).staticmethod("_setDataOnNode")
...
struct DemField: public Field{
	...
	#define woo_dem_DemField__CLASS_BASE_DOC_ATTRS_CTOR_PY \
		DemField,Field,ClassTrait().doc("Field describing a discrete element assembly. Each particle references (possibly many) nodes.\n\n.. admonition:: Special constructor\n\n\tWhen passed the ``par`` parameter in constructor, this sequence of particles will be assigned to :obj:`particles` and nodes created as with :obj:`S.dem.add <ParticleContainer.add>`. An additional optional parameter ``parNodes`` will be passed as *nodes* to :obj:`S.dem.add(...,nodes=...) <ParticleContainer.add>` to determine which nodes are to be added.").section("DEM field","TODO",{"ContactContainer","ParticleContainer"}), \
		...
		.def_static("sceneHasField",&Field_sceneHasField<DemField>).staticmethod("sceneHasField") \
		.def_static("sceneGetField",&Field_sceneGetField<DemField>).staticmethod("sceneGetField"); \
