...
struct LudingPhys: public FrictPhys{
	...
	void addWork(int type, const Real& dW);
	void commitWork(const Scene* scene, const Particle* pA, const Particle* pB, int& viscIx, int& plastIx) const;
	#define woo_dem_LudingPhys__CLASS_BASE_DOC_ATTRS \
	...
	WOO_DECL__CLASS_BASE_DOC_ATTRS(woo_dem_LudingPhys__CLASS_BASE_DOC_ATTRS);
};
