...
GLViewer::GLViewer(int _viewId): QGLViewer(/*parent*/(QWidget*)NULL), viewId(_viewId) {
	...
	setKeyDescription(Qt::Key_C & (int)Qt::AltModifier,"Set scene center to median body position (same as space)");
	setKeyDescription(Qt::Key_D,"Toggle time display mask");
	setKeyDescription(Qt::Key_D & (int)Qt::ShiftModifier,"Toggle local date/time display");
	setKeyDescription(Qt::Key_F,"Toggle Scene center following Scene.gl.renderer.centNode (if defined)");
	setKeyDescription(Qt::Key_F & (int)Qt::ShiftModifier,"Set S.gl.renderer.centNode to currently selected node (S.gl.renderer.selObjNode) and switch following.");
	setKeyDescription(Qt::Key_G,"Cycle through visible grid planes");
	setKeyDescription(Qt::Key_G & (int)Qt::ShiftModifier ,"Toggle grid visibility.");
	...
	setKeyDescription(Qt::Key_S & (int)Qt::ShiftModifier,"Set reference positions for displacements scaling to the current ones.");
	setKeyDescription(Qt::Key_S & (int)Qt::AltModifier,"Save QGLViewer state to /tmp/qglviewerState.xml");
	setKeyDescription(Qt::Key_S & (int)Qt::ControlModifier,"Save hardcopy of the image to file.");
	setKeyDescription(Qt::Key_C & (int)Qt::ControlModifier,"Save hardcopy of the image to clipboard.");
	...
	setKeyDescription(Qt::Key_Q & (int)Qt::ShiftModifier,"Toggle fast rendering mode (always, unfocused, never)");
	#if 0
	...
	setKeyDescription(Qt::Key_1 & (int)Qt::AltModifier,"Add/remove plane #1 to/from the bound group");
	setKeyDescription(Qt::Key_2 & (int)Qt::AltModifier,"Add/remove plane #2 to/from the bound group");
	setKeyDescription(Qt::Key_3 & (int)Qt::AltModifier,"Add/remove plane #3 to/from the bound group");
