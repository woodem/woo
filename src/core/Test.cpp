...
void WooTestClass::arr3d_set(const Vector3i& shape, const vector<Real>& data){
	if((int)data.size()!=shape.prod()) throw std::runtime_error("Data must have exactly "+to_string(shape[0])+"x"+to_string(shape[1])+"x"+to_string(shape[2])+"="+to_string(shape.prod())+" items (not "+to_string(data.size())+").");
