...
namespace aabb
{
	...
	AABB::AABB(const VecT& lowerBound_, const VecT& upperBound_) :
	...
	Scalar AABB::computeSurfaceArea() const
	{
	...
	Scalar AABB::getSurfaceArea() const
	{
		return surfaceArea;
	}
	...
	VecT AABB::computeCentre()
	{
		VecT position(lowerBound.size());
	...
	Tree::Tree(Scalar skinThickness_,
		...
		touchIsOverlap(touchIsOverlap_)
	{
		...
		periodicity=VecBoolT::Constant(false);
		...
		touchIsOverlap(touchIsOverlap_)
	{
		...
		// Check periodicity.
		#if 1
			isPeriodic=periodicity.array().any();
			posMinImage= .5*boxSize;
	...
	void Tree::setPeriodicity(const VecBoolT& periodicity_)
	{
		periodicity = periodicity_;
	}

	void Tree::setBoxSize(const VecT& boxSize_)
	{
		boxSize = boxSize_;
	}
	...
	void Tree::insertParticle(unsigned int particle, const VecT& lowerBound, const VecT& upperBound)
	{
		...
		VecT size;
		#if 1
			if((lowerBound.array()>upperBound.array()).any()) throw std::invalid_argument("[ERROR]: AABB lower bound is greater than the upper bound!");
	...
	bool Tree::updateParticle(unsigned int particle, const VecT& lowerBound,
							  const VecT& upperBound, bool alwaysReinsert)
	{
		// Map iterator.
		std::unordered_map<unsigned int, unsigned int>::iterator it;
	...
	Scalar Tree::computeSurfaceAreaRatio() const
	{
