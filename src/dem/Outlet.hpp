...
struct Outlet: public PeriodicEngine{
	...
	#define woo_dem_Outlet__CLASS_BASE_DOC_ATTRS_PY \
		...
		((vector<Real>,rDivR0,,AttrTrait<>().noGui().noDump().readonly(),"List of the :math:`r/r_0` ratio of deleted particles, when :obj:`recoverRadius` is true.")) \
		...
		((vector<int>,locs,,AttrTrait<>().noGui().noDump().readonly(),"Integer location specified for particles; -1 by default, derived classes can use this for any purposes (usually more precise location within the outlet volume).")) \
