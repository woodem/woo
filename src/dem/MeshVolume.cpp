...
void MeshVolume::init(){
	...
	for(const shared_ptr<Particle>& p: *(field->cast<DemField>().particles)){
		...
		LOG_DEBUG("Facet #{}, mask={} (mesh mask {}",p->id,p->mask,mask);
		...
		for(int i:{0,1,2}){
			...
			if(nI==nodeIx.end()){
				LOG_TRACE("Node at {} not yet seen in this surface, adding to [{}]",n->pos.transpose(),nodes.size());
				...
				LOG_TRACE("Node at {} already known, at [{}]",n->pos.transpose(),nI->second);
				nIxs[i]=nI->second;
		...
		for(int i:{0,1,2}){
			...
			if(nnI==edgeIx.end()){ // edge not created yet
				LOG_TRACE("Edge between {} and {} not yet seen, adding to {}]",ni->pos.transpose(),ni1->pos.transpose(),edges.size());
				...
				LOG_TRACE("Edge between {} and {} already known, at [{}]",ni->pos.transpose(),ni1->pos.transpose(),nnI->second);
				eIxs[i]=nnI->second;
			}
		}
	...
	LOG_INFO("Create surface with {} vertices, {} edges, {} faces. The surface is{} orientable, is{} closed.",gts_surface_vertex_number(surface.get()),gts_surface_edge_number(surface.get()),gts_surface_face_number(surface.get()),(gts_surface_is_orientable(surface.get())?"":" NOT"),(gts_surface_is_closed(surface.get())?"":" NOT"));
