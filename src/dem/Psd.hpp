...
struct PsdSphereGenerator: public ParticleGenerator{
	...
	#define woo_dem_PsdSphereGenerator__CLASS_BASE_DOC_ATTRS_PY \
		...
		, /* py */ \
			.def("inputPsd",&PsdSphereGenerator::pyInputPsd,WOO_PY_ARGS(py::arg("normalize")=true,py::arg("cumulative")=true,py::arg("num")=80),"Return input PSD; it will be a staircase function if *discrete* is true, otherwise linearly interpolated. With *normalize*, the maximum is equal to 1.; otherwise, the curve is multiplied with the actually generated mass/numer of particles (depending on whether :obj:`mass` is true or false); the result should then be very similar to the :obj:`woo.dem.ParticleGenerator.psd` output with actually generated spheres. Discrete non-cumulative PSDs are handled specially: discrete distributions return skypline plot with peaks represented as plateaus of the relative width 1/*num*; continuous distributions return ideal histogram computed for relative bin with 1/*num*; thus returned histogram will match non-cummulative histogram returned by :obj:`ParticleGenerator.psd(cumulative=False) <woo.dem.ParticleGenerator.psd>`, provided *num* is the same in both cases.\n\n .. todo:: with ``cumulative=False`` and when the PSD does not :math:`x`-start at 0, the plot is not correct as the hump coming from the jump in the PSD (from 0 to non-zero at the left) is not taken in account.")

	WOO_DECL__CLASS_BASE_DOC_ATTRS_PY(woo_dem_PsdSphereGenerator__CLASS_BASE_DOC_ATTRS_PY);
};
...
struct PsdClumpGenerator: public PsdSphereGenerator {
	...
	Real critDt(Real density, Real young) override;
	bool isSpheresOnly() const override { return true; }
	enum {SCALE_EQUIVRAD=0,SCALE_FIRSTSPHERE,SCALE_AVGSPHERE};
	#define woo_dem_PsdClumpGenerator__CLASS_BASE_DOC_ATTRS \
		PsdClumpGenerator,PsdSphereGenerator,"Generate clump particles following a given Particle Size Distribution. !! FULL DOCUMENTATION IS IN py/_monkey/extraDocs.py !!!", \
		...
		((int,scaleMethod,SCALE_EQUIVRAD,AttrTrait<Attr::namedEnum>().namedEnum({{SCALE_EQUIVRAD,{"equiv","equivRad"}},{SCALE_FIRSTSPHERE,{"firstSphere"}},{SCALE_AVGSPHERE,{"avgSphere"}}}),"Method to compute clump scaling. The default uses the equivalent radius to determine scaling to fulfill prescribed PSD. Other methods are to scale according to the radius of the first sphere in the clump, or according to the average sphere radius in the clump.")) \
	...
	WOO_DECL__CLASS_BASE_DOC_ATTRS(woo_dem_PsdClumpGenerator__CLASS_BASE_DOC_ATTRS);
};
...
struct PsdCapsuleGenerator: public PsdSphereGenerator {
	...
	WOO_DECL__CLASS_BASE_DOC_ATTRS(woo_dem_PsdCapsuleGenerator__CLASS_BASE_DOC_ATTRS);
};
...
struct PsdEllipsoidGenerator: public PsdSphereGenerator {
	...
	bool isSpheresOnly() const override { return false; }
	// clear, critDt: same as for PsdSphereGenerator
	#define woo_dem_PsdEllipsoidGenerator__CLASS_BASE_DOC_ATTRS \
		PsdEllipsoidGenerator,PsdSphereGenerator,"Generate ellipsoids following a given Particle Size Distribution; ratio of :obj:`Ellipsoid.semiAxes` is chosen randomly from the :obj:`semiAxesRatio` range; orientation is random.", \
	...
	WOO_DECL__CLASS_BASE_DOC_ATTRS(woo_dem_PsdEllipsoidGenerator__CLASS_BASE_DOC_ATTRS);
};
