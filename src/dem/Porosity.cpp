...
Real AnisoPorosityAnalyzer::relSolid(Real theta, Real phi, Vector3r pt0, bool vis){
	...
	for(size_t i=0; i<sz; i++){
		...
		#ifdef WOO_DEBUG
			for(int j:{0,1,2}) if(A[j]<0 || A[j]>scene->cell->getSize()[j] || B[j]<0 || B[j]>scene->cell->getSize()[j]) throw std::logic_error(fmt::format("Points outside periodic cell after splitting? {} {} (cell size {})",A.transpose(),B.transpose(),scene->cell->getSize().transpose()));
		#endif
...
Real AnisoPorosityAnalyzer::computeOneRay_angles_check(Real theta, Real phi, bool vis){
	...
	// theta=azimuth, phi=elevation
	if(theta<0 || theta>M_PI/2. || phi<0 || phi>M_PI/2.) throw std::invalid_argument(fmt::format("AnisoPorosityAnalyzer.oneRay: value out of valid range: theta={} (0ɹ/2), phi={} (0ɹ/2)",theta,phi));
