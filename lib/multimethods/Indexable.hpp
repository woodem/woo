...
#define REGISTER_CLASS_INDEX(SomeClass,BaseClass)                                      \
	...
	virtual int& getClassIndex() override { return getClassIndexStatic(); }        \
	virtual const int& getClassIndex() const override { return getClassIndexStatic(); }  \
	virtual int& getBaseClassIndex(int depth) override {              \
...
#define REGISTER_INDEX_COUNTER(SomeClass) \
	...
	virtual const int& getClassIndex() const override { return getClassIndexStatic(); }       \
	virtual int& getBaseClassIndex(int) override { static int _1(-1); return _1; /* throw std::logic_error("One of the following errors was detected:\n(1) Class " #SomeClass " called createIndex() in its ctor (but it shouldn't, being a top-level indexable; only use REGISTER_INDEX_COUNTER, but not createIndex()).\n(2) Some DerivedClass deriving from " #SomeClass " forgot to use REGISTER_CLASS_INDEX(DerivedClass," #SomeClass ").\nPlease fix that and come back again." );*/} \
	virtual const int& getBaseClassIndex(int) const override { static int _1(-1); return _1; /* throw std::logic_error("One of the following errors was detected:\n(1) Class " #SomeClass " called createIndex() in its ctor (but it shouldn't, being a top-level indexable; only use REGISTER_INDEX_COUNTER, but not createIndex()).\n(2) Some DerivedClass deriving from " #SomeClass " forgot to use REGISTER_CLASS_INDEX(DerivedClass," #SomeClass ").\nPlease fix that and come back again." ); */ } \
	virtual const int& getMaxCurrentlyUsedClassIndex() const override { assert(dynamic_cast<SomeClass*>(const_cast<SomeClass*>(this))); return getMaxCurrentlyUsedIndexStatic(); } \
